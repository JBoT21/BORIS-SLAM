...
#include "SlamFunctionSet.h" // for SLAM
...
enum ConquerorCarFunctionalModel
{
  ...
  SLAM_MAPPING_MODE = 16,
  SLAM_NAVIGATION_MODE = 17,
...
void ApplicationFunctionSet::ApplicationFunctionSet_RGB(void)
{
  ...
  else if (((function_xxx((temp), 500, 3000)) && VoltageDetectionStatus == true) || VoltageDetectionStatus == false)
  {
    ...
    case /* constant-expression */ Rocker_mode:
      ...
      case /* constant-expression */ SLAM_MAPPING_MODE:
      /* code */
      Application_ConquerorCarxxx0.Functional_Mode = SLAM_NAVIGATION_MODE;
      break;
...
void ApplicationFunctionSet::ApplicationFunctionSet_SLAMMode() { //SLAM
    if (Application_ConquerorCarxxx0.Functional_Mode == SLAM_MAPPING_MODE) {
        // Call SLAM mapping logic
        SLAM_FunctionSet.executeMapping();
    } else if (Application_ConquerorCarxxx0.Functional_Mode == SLAM_NAVIGATION_MODE) {
        // Call SLAM navigation logic
